...
export function calculateCustomVectorDeformedPoints(
  ...
  const nodeDisplacements = nodes.map(n => ({
    ...
    dx: n.x - n.origX,
    dy: n.y - n.origY,
    rad: Math.min(35, Math.max(12, n.radius || captureRadius || 25))
  }));
    ...
    if (parent && parent.node.id !== curr.node.id) {
      ...
      if (!processedPairs.has(pairKey)) {
        ...
        segments.push({
          ...
          dxB: curr.dx,
          dyB: curr.dy,
          rad: Math.min(35, Math.max(12, Math.max(parent.rad, curr.rad)))
        });
      }
    }
  }
  ...
  return origPoints.map(pt => {
    ...
    return {
      ...
      ...(ext.p2 ? { p2: { x: Number((ext.p2.x + dispY).toFixed(2)), y: Number((ext.p2.y + dispY).toFixed(2)) } } : {})
    };
  });
}
