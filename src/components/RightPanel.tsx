...
export default function RightPanel({
  ...
  React.useEffect(() => {
    ...
    if (activeLasso) {
      ...
      (Object.values(objectsRef.current) as VectorObject[]).forEach(rawObj => {
        let obj = rawObj;
        if (rawObj.type === '360_container' && rawObj.views360) {
          const activeView = findClosestView360(rawObj.views360, rawObj.currentAngle360 ?? 0);
          if (activeView && objectsRef.current[activeView.drawingId]) {
            obj = objectsRef.current[activeView.drawingId];
          }
        }
  ...
  const isLassoActive = (!!lassoPoints && lassoPoints.length >= 3) || !!targetObject?.lassoDeformState?.active;
  ...
  const isMeshPointActive = !!(
    (activeTool === 'PBM' || activeTool === 'VDF' || activeTool === 'VPR' || activeTool === 'RPD' || vdfState?.active) &&
    targetObject &&
    vdfState &&
  ...
  return (
    <div
      className={`absolute right-0 top-0 bottom-0 h-full transition-all duration-200 shrink-0 z-30 ${
        open ? 'w-80' : 'w-0'
      }`}
    >
      ...
      <div className={`w-full h-full bg-neutral-900/95 backdrop-blur-md border-l border-neutral-800 flex flex-col overflow-hidden box-border min-w-0 ${
                      ...
                      <div className="space-y-4 text-xs">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-col gap-2.5">
                            <div className="flex items-center justify-between bg-neutral-900/60 p-2 rounded-xl border border-neutral-800/80">
                              <span className="text-[10px] text-neutral-300 font-bold uppercase tracking-wider">Color Picker</span>
                              <CustomColorPicker 
                                color={brushColor} 
                                onChange={(val) => {
                                  setBrushColor(val);
                            ...
                            <div className="space-y-1.5 w-full">
                              <span className="text-[10px] text-amber-400 font-extrabold uppercase tracking-wider block">
                                ✨ Mesh Paint & Glow Mode
                              </span>
                              <div className="flex flex-col gap-1.5 bg-neutral-950 p-2 rounded-xl border border-neutral-800/60 w-full">
                                <button
                                  type="button"
                                  onClick={() => handleUpdateMeshColorConfig({ paintMode: 'cell' })}
                                  className={`py-2 px-3 text-xs font-bold rounded-lg transition-all text-left flex items-center justify-between cursor-pointer ${
                                ...
                                >
                                  <span className="flex items-center gap-1.5">🎨 Fill Cell Paint</span>
                                  {selectedObject.smartMeshColor.paintMode === 'cell' && <span className="text-[10px] font-black">✓ ACTIVE</span>}
                                </button>
                                <button
                                  type="button"
                                ...
                                >
                                  <span className="flex items-center gap-1.5">✨ Glow Point Paint (Neon Aura)</span>
                                  {selectedObject.smartMeshColor.paintMode === 'point' && <span className="text-[10px] font-black">✓ ACTIVE</span>}
                                </button>
                              </div>
                            </div>
                          </div>
